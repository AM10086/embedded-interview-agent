...
- **网页版简历 AI 模拟面试**：上传简历（支持 .txt / .md / **PDF**）+ 目标岗位 → 大厂审问式 20 题面试（层层追问/挑战式提问/STAR 深挖/综合压力题）→ 面试分析/修正计划/简历优化/学习计划；内置审问引擎免 Key 开箱即用，配置大模型 API Key 后全真驱动
- **代码类面试题 200 道**：代码判断（找 Bug）/ Linux 命令 / 代码分析（输出与行为）/ 代码写作（手写代码），代码块高亮展示、附参考代码
- **简历项目驱动面试**：自动解析简历中的项目/技能/协议，重点进行 STAR 项目深挖、问题排查（遇到难点怎么解决）、通信协议对比与选型（SPI/I2C/UART/CAN/USB/RS485 等），并已接入智能练习/题库目录
- **智能判分**：判断/选择类题目真正判定对错（答错明显扣分并提示正确答案），按核心术语、结论先行、数据与取舍多维评分，每题附参考答案要点
- **用户登录**：注册/登录账号，各账号的练习记录、薄弱点、AI 配置完全隔离，互不干扰
...
> 纯前端实现，支持按薄弱点出题、面试模拟、薄弱点分析、**简历 AI 模拟面试**（上传 txt/md/**PDF** 简历 → 20 题大厂审问式面试 → 四维报告）、**用户登录与数据隔离**，数据与 Python 版共用同一题库。
